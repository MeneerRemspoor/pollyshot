# Pollyshot

*No copy. No paste. No friction.*

![pollyshot-small](https://github.com/user-attachments/assets/77699573-8688-40b2-8e83-5b0a66583592)

## The problem

Every day we take dozens of screenshots. The usual flow: take a screenshot → find the file or wait for it to land on the clipboard → switch to the target app → paste. Repeat for every screenshot, every app, every time.
Pollyshot eliminates those steps. One shortcut, one selection, done.

## How it works?

- Configure up to 10 of your favorite apps
- Trigger Pollyshot with a keyboard shortcut (or via the menu bar dropdown)
- Select the area of your screen you want to capture
- Pollyshot instantly pastes it into your target app — no clipboard involved

<img width="777" height="499" alt="afbeelding" src="https://github.com/user-attachments/assets/382019fa-495e-4cb1-b0f0-9c5304683666" />

## Getting started

### Installation
1. Download and install Pollyshot on macOS
2. Grant the required permissions:
   - Go to *System Settings > Privacy & Security > Accessibility* and add Pollyshot
   - Go to *System Settings > Privacy & Security > Screen & System Audio Recording* and add Pollyshot

<img width="400" alt="Pollyshot settings - Accessibility" src="https://github.com/user-attachments/assets/619e4c16-8833-471e-8054-42339749b27b" />
<img width="400" alt="Pollyshot settings - Screen and System Audio Recording" src="https://github.com/user-attachments/assets/e61d27b0-c9a5-4e4f-9205-d1168101a6b1" />

## Supported apps
Pollyshot has been tested with the following apps:
ChatGPT, Claude, Slack, Figma, WhatsApp, Viber, Cursor, Zed, Apple Notes, Apple Pages

Other applications may work too — support will broaden as development continues.

## Status
Pollyshot is currently in beta. More functionality is on the way. If you run into issues with a specific app, please open an issue.

## Requirements
- macOS
- Accessibility permission
- Screen & System Audio Recording permission
